  ...
  return (
    ...
    >
      <Link
        ...
        className="group relative block rounded-xl border border-slate-200 bg-white p-5 transition-shadow hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
      >
        {onToggleWatch && (
          <button
            type="button"
            ...
            className="absolute right-4 top-4 rounded-full p-1 text-slate-300 transition-colors hover:text-amber-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          >
        ...
        <div className="flex items-start justify-between gap-3 pr-6">
          ...
          <Badge variant="outline" className="shrink-0 text-slate-600">
            {company.latestFilingType}
          </Badge>
        </div>

        <p className="mt-3 text-xs text-slate-400">{formatFilingDate(company.latestFilingDate)} 제출</p>
        ...
        <div className="mt-4 flex items-start gap-2 border-t border-slate-100 pt-3">
          ...
          />
          <div className="min-w-0">
            <p className="line-clamp-2 text-sm text-slate-700">{company.changeSummary}</p>
            {level !== 'flat' && dominant.key && (
              <p className={`mt-1 text-xs font-medium ${style.text}`}>
                {SCORE_COMPONENT_LABELS[dominant.key]} 변동 감지
              </p>
            )}
          </div>
        </div>
