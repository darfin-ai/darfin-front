...
export function SubscriptionManagement() {
  ...
  const handleCancelSubscription = async () => {
    if (!confirm(t("account.subscription.cancelConfirm", { date: formatLocaleDate(subscription?.nextPaymentDate, locale) }))) return;
    setIsChangingPlan(true);
    try {
      ...
      <div className="mb-8 max-w-2xl">
        <nav className="flex items-center gap-1.5 text-sm text-slate-400 dark:text-slate-500 mb-4">
          <Link to="/mypage" className="hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
            {t("account.subscription.breadcrumbMypage")}
          </Link>
          <ChevronRight size={14} aria-hidden />
          <span className="text-slate-600 dark:text-slate-400">{t("account.subscription.breadcrumbCurrent")}</span>
        </nav>
        <div className={EYEBROW}>{t("account.subscription.eyebrow")}</div>
        <h1 className={PAGE_TITLE}>{t("account.subscription.title")}</h1>
        <p className={`${PAGE_DESC} mt-2`}>{t("account.subscription.description")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className={`${CARD} p-5`}>
            <div className="flex items-center justify-between mb-4">
              ...
              >
                ...
                {isCancelScheduled ? t("account.subscription.statusCancelScheduled") : t("account.subscription.statusActive")}
              </span>
            </div>
            ...
            <div className="space-y-2.5 mb-5 text-sm text-slate-500 dark:text-slate-400">
              {subscription?.nextPaymentDate && (
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {isCancelScheduled ? t("account.subscription.serviceEndDate") : t("account.subscription.nextPayment")}:{" "}
                    <span className="font-medium text-slate-700 dark:text-slate-200">
                      {formatLocaleDate(subscription.nextPaymentDate, locale)}
                    </span>
                  </span>
                </div>
              )}
              <div className="flex items-center gap-2">
                ...
                <span>
                  {t("account.subscription.paymentMethod")}:{" "}
                  <span className="font-medium text-slate-700 dark:text-slate-200">
                    {defaultMethod ? `${defaultMethod.cardCompany} ${defaultMethod.maskedCardNum}` : t("account.subscription.noPaymentMethod")}
                  </span>
                </span>
              </div>
            </div>
            ...
            {currentPlanName !== "BASIC" && (
              isCancelScheduled ? (
                ...
                >
                  {t("account.subscription.cancelScheduled")}
                </button>
              ) : (
                <button
                  type="button"
          ...
          <div className={`${CARD} p-5`}>
            <h2 className={`${SECTION_TITLE} mb-4 flex items-center gap-2`}>
              ...
              {t("account.subscription.paymentMethodsTitle")}
            </h2>
                ...
                >
                  ...
                  {method.isDefault && (
                    <span className="shrink-0 rounded-full bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-300 border border-blue-200 dark:border-blue-800 text-[10px] font-medium px-1.5 py-0.5">
                      {t("account.subscription.defaultBadge")}
                    </span>
                  )}
                </div>
              ))}
              {paymentMethods.length === 0 && (
                <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">{t("account.subscription.noMethods")}</p>
              )}
            </div>
            ...
            >
              <Plus size={16} />
              {cardRegistering ? t("account.subscription.redirecting") : t("account.subscription.addMethod")}
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className={`${CARD} p-5 sm:p-6`}>
            <h2 className={`${SECTION_TITLE} mb-5`}>{t("account.subscription.changePlanTitle")}</h2>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          {plan.planName === currentPlanName && (
                            <span className="rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-medium px-1.5 py-0.5">
                              {t("account.subscription.currentPlanBadge")}
                            </span>
                          )}
                        </div>
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 p-4 mb-6 flex gap-3">
              ...
              <div className="text-sm text-slate-500 dark:text-slate-400">
                <p className="font-medium text-slate-700 dark:text-slate-200 mb-1.5">{t("account.subscription.billingNotesTitle")}</p>
                <ul className="space-y-1 text-xs leading-relaxed">
                  {t("account.subscription.billingNotes").map((note, index) => (
              ...
              >
                {isChangingPlan ? (
                  <>
                    ...
                    {t("account.subscription.processing")}
                  </>
                ) : isSamePlanSelected ? (
                  t("account.subscription.samePlan")
                ) : selectedPlanId === "BASIC" ? (
                  t("account.subscription.switchToBasic")
                ) : (
                  <>
                    {t("account.subscription.changeToSelected")}
                    <ArrowRight size={16} />
                  </>
                )}
              </button>
